...
class Shipr extends Orchestrator {
  ...
  /**
   * Run command in one function
   *
   * @param {String|Object|Array} command command or options or array of options
   * @param {Object} [options]
  ...
  run(command, options) {
    if (Array.isArray(command)) {
      return PromiseA.mapSeries(command, c => this._run(c));
    } else {
      return this._run(command, options);
    }
  }
  ...
  _run(command, options) {
    ...
    const {local, env, batch} = options || {};
    const runner = local ? this.local : this.remote;
    ...
    if (env) {
      ...
      if (names && names.length) {
        command = names.map(name => name + '=' + env[name]).join(' ') + ' ' + command
      }
    }

    // TODO generate script file and send it to remote and
    this.log(pad(`\nRUN: [${options.name || (command.substr(0, 10) + ' ...')}] `, process.stdout.columns || 100, '*'));
    return runner.call(this, command, options);
  }
